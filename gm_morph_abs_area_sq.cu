...
__device__ double area_squares_abs_difference(int limit, int step, std::uint16_t* d_image, int ind1, int ind2) {
    double asd = 0LL, band_diff;
...
__global__ void gm_morph_abs_area_sq_kernel(int NX, int NY, int NC, std::uint16_t* d_image, double* d_gm) {
    int i, j, ind;
    int pi, pj, pind;
    int qi, qj, qind;
    double gm = 0LL;
            ...
            if (pi >= 0 && pi < NX && pj >= 0 && pj < NY) {
                ...
                for (int q = p + 1; q < 9; q++) {
                    ...
                    if (qi >= 0 && qi < NX && qj >= 0 && qj < NY) {
                        qind = qi + qj * NX;
                        double asd = area_squares_abs_difference(NX * NY * NC, NX * NY, d_image, pind, qind);
                        if (asd > gm) {
                            gm = asd;
                        }
                    }
...
void gm_morph_abs_area_sq(int NX, int NY, int NC, std::uint16_t* h_image, double* h_gm) {
    // initialisations
    int bx, by;
    ...
    gm_morph_abs_area_sq_kernel<<<dimGrid, dimBlock>>>(NX, NY, NC, d_image, d_gm);
