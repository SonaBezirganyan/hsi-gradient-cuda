...
__global__ void gm_gray_sum_kernel(int NX, int NY, int NC, std::uint16_t* d_image, double* d_gm) {
    int i, j, ind;
    ...
    if (i < NX && j < NY) { // && i >= 0 && j >= 0
        ...
        for (int c = 0; c < NC; ++c) {
            ...
            sum_magnitude += sqrt(dx * dx + dy * dy);
        }
        d_gm[ind] = sum_magnitude;
    }
}

void gm_gray_sum(int NX, int NY, int NC, std::uint16_t* h_image, double* h_gm) {
    // initialisations
    int bx, by;
    ...
    gm_gray_sum_kernel<<<dimGrid, dimBlock>>>(NX, NY, NC, d_image, d_gm);
