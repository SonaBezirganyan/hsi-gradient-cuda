...
__device__ int area_abs_difference(int limit, int step, std::uint16_t* d_image, int ind1, int ind2) {
    int asd = 0, band_diff;
...
__global__ void gm_morph_abs_area_kernel(int NX, int NY, int NC, std::uint16_t* d_image, int* d_gm) {
    int i, j, ind;
    int pi, pj, pind;
    int qi, qj, qind;
    int gm = 0;
            ...
            if (pi >= 0 && pi < NX && pj >= 0 && pj < NY) {
                ...
                for (int q = p + 1; q < 9; q++) {
                    ...
                    if (qi >= 0 && qi < NX && qj >= 0 && qj < NY) {
                        qind = qi + qj * NX;
                        int asd = area_abs_difference(NX * NY * NC, NX * NY, d_image, pind, qind);
                        if (asd > gm) {
                            gm = asd;
                        }
                    }
...
void gm_morph_abs_area(int NX, int NY, int NC, std::uint16_t* h_image, int* h_gm) {
    // initialisations
    int bx, by;
    ...
    gm_morph_abs_area_kernel<<<dimGrid, dimBlock>>>(NX, NY, NC, d_image, d_gm);
