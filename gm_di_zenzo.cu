...
__global__ void gm_di_zenzo_kernel(int NX, int NY, int NC, std::uint16_t* d_image, double* d_gm) {
    int i, j, ind;
    ...
    if (i < NX && j < NY) { // && i >= 0 && j >= 0
        ...
        double delta = sqrt((g_xx - g_yy) * (g_xx - g_yy) + 4 * g_xy * g_xy);
        d_gm[ind] = sqrt((g_xx + g_yy + delta) / 2);
    }
}

void gm_di_zenzo(int NX, int NY, int NC, std::uint16_t* h_image, double* h_gm) {
    // initialisations
    int bx, by;
    ...
    gm_di_zenzo_kernel<<<dimGrid, dimBlock>>>(NX, NY, NC, d_image, d_gm);
