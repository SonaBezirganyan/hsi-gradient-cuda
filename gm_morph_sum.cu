...
__global__ void gm_morph_sum_kernel(int NX, int NY, int NC, std::uint16_t* d_image, int* d_gm) {
    int i, j, ind;
    int ni, nj, nind;
    ...
    if (i < NX && j < NY) { // && i >= 0 && j >= 0
        ...
        for (int c = 0; c < NC; c++) {
            ...
            sum_marginal += max_val - min_val;
        }
        d_gm[ind] = sum_marginal;
    }
}


void gm_morph_sum(int NX, int NY, int NC, std::uint16_t* h_image, int* h_gm) {
    // initialisations
    int bx, by;
    ...
    gm_morph_sum_kernel<<<dimGrid, dimBlock>>>(NX, NY, NC, d_image, d_gm);
