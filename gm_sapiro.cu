...
__global__ void gm_sapiro_kernel(int NX, int NY, int NC, std::uint16_t* d_image, double* d_gm) {
    int i, j, ind;
...
void gm_sapiro(int NX, int NY, int NC, std::uint16_t* h_image, double* h_gm) {
    // initialisations
    int bx, by;
    ...
    gm_sapiro_kernel<<<dimGrid, dimBlock>>>(NX, NY, NC, d_image, d_gm);
